...
func (o *ProtocolsSet) GetPeers() PeerSlice {
	if o == nil {
		return nil
	}
...
func (o *ProtocolsSet) GetVisits() VisitSlice {
	if o == nil {
		return nil
	}
...
func (r *protocolsSetR) GetVisits() VisitSlice {
	if r == nil {
		return nil
	}
...
func (protocolsSetL) LoadPeers(ctx context.Context, e boil.ContextExecutor, singular bool, maybeProtocolsSet any, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`peers`),
		qm.WhereIn(`peers.protocols_set_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (protocolsSetL) LoadVisits(ctx context.Context, e boil.ContextExecutor, singular bool, maybeProtocolsSet any, mods queries.Applicator) error {
	var slice []*ProtocolsSet
	var object *ProtocolsSet
	...
	query := NewQuery(
		qm.From(`visits`),
		qm.WhereIn(`visits.protocols_set_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *ProtocolsSet) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(protocolsSetAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(protocolsSetPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert protocols_sets, could not build conflict column list")
			}

			conflict = make([]string, len(protocolsSetPrimaryKeyColumns))
			copy(conflict, protocolsSetPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"protocols_sets\"", updateOnConflict, ret, update, conflict, insert, opts...)
